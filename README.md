...
### 1.11 Appendix III - How do we get $\Gamma = 2\pi R^2 \Omega$
...
$$
\Gamma = \int_0^{2\pi} (R\Omega) R d\theta.
$$

Since $R$ and $\Omega$ are constants,

$$
\Gamma = R^2 \Omega \int_0^{2\pi} d\theta.
$$
...
$$
\Gamma = \oint V_\theta ds = (R\Omega)(2\pi R) = 2\pi R^2 \Omega.
$$

- Or using the vortex representation:

$$
\frac{\Gamma}{2\pi R} = R\Omega \implies \Gamma = 2\pi R^2 \Omega.
$$

#### 1.11.2 One important qualification

I would not present $\Gamma = 2\pi R^2 \Omega$ as a universal law for a real rotating cylinder. A real fluid has viscosity and a boundary layer, and the actual circulation generated by a spinning cylinder depends on the flow conditions.
...
#### 2.4.1 Circulation for Joukowski Airfoil: Derivation of $\Gamma = 4\pi R U_\infty \sin(\alpha + \beta)$
...
$$
V_\theta(\theta) = -2 U_\infty \sin(\theta - \alpha) - \frac{\Gamma}{2\pi R}
$$
...
$$
-2 U_\infty \sin(\theta_{\text{stag}} - \alpha) - \frac{\Gamma}{2\pi R} = 0
$$

Rearranging for $\Gamma$:

$$
\Gamma = -4 \pi R U_\infty \sin(\theta_{\text{stag}} - \alpha)
$$

Using $\sin(-x) = -\sin(x)$, we get:

$$
\Gamma = 4 \pi R U_\infty \sin(\alpha - \theta_{\text{stag}})
$$
...
$$
\tan\beta = \frac{\mu_y}{c + \mu_x} \approx \frac{\mu_y}{R}
$$
...
$$
\Gamma = 4 \pi R U_\infty \sin\Big(\alpha - (-\beta)\Big)
$$
...
$$
\bbox{\Gamma = 4 \pi R U_\infty \sin(\alpha + \beta)}
$$
