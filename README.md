...
- **Top Surface ($0 < \theta < \pi$)**: On the top surface, the clockwise circulation adds a velocity component in the negative $\theta$ direction (toward the trailing edge). This combines with the uniform flow's own negative component, making the total velocity negative (and faster in magnitude).
...
---
## 3. Why Wings Create Lift: The Role of the Starting Vortex and Kutta Condition
...
This is where Kelvin's Circulation Theorem comes in. Strictly speaking, this theorem states that "for an inviscid, barotropic flow under conservative forces only, the circulation around a closed curve moving with the fluid particles does not change with time." Near the thin boundary layer close to the wing surface, this theorem doesn't strictly hold due to viscous effects, but for a large closed curve encompassing the entire flow field far from the wing, it holds approximately well. Therefore, the total circulation (Γ — the integral of velocity along a closed curve, a concept distinct from angular momentum) calculated along this large curve must remain zero, just as it was when the system was initially at rest. As a result, a Bound Vortex with circulation in the exact opposite direction forms around the wing, canceling out the circulation of the starting vortex left behind in the wake.  

![Bound Vortex](images/Shed_vortex.png) 
...
Substitute the Kutta condition ($\Gamma = -4\pi a U_{\infty} \sin(\alpha + \beta)$) to get the explicit lift:
